...
const TOAST_LIMIT = 1;
const TOAST_REMOVE_DELAY = 300; // Quick removal animation
...
// Default duration for different toast types
...
function toast({ duration, ...props }: Toast) {
  ...
  const autoDismissDuration = duration ?? DEFAULT_DURATION;
...
// Dismiss all toasts on scroll - useful for admin notifications
...
function setupScrollListener() {
  ...
  const handleScroll = () => {
    ...
    // Dismiss all toasts if user scrolls more than 50px
    if (scrollDelta > 50 && memoryState.toasts.length > 0) {
