...
for migration in chatsev/supabase/migrations/*.sql; do
    if [ -f "$migration" ]; then
...
rsync -av --update chatsev/supabase/functions/ supabase-edge-functions:/home/deno/functions/ || \
    docker cp chatsev/supabase/functions/. supabase-edge-functions:/home/deno/functions/
